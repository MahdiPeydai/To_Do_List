...
from app.helpers.get_username_by_id import get_username
from app.helpers.forms import CreateCategoryForm
from app.helpers.forms import CreateActivityForm
import os
import json
...
def dashboard_page(user_id):
    ...
    with open('app/data/users_categories.json', 'r') as categories_file:
        categories = json.load(categories_file)

    with open('app/data/users_activities.json', 'r') as activities_file:
    ...
    return render_template('dashboard/dashboard.html', user_name=user_name, user_categories=user_categories, user_activities=user_activities)


@dashboard.route('/dashboard/user/category/<int:category_id>/activity/<int:activity_id>/status/update')
@token_check
def status_changer(user_id, category_id, activity_id):
    ...
    with open('app/data/users_activities.json', 'w+') as activities_file:
        json.dump(activities, activities_file, indent=2)

    return redirect(url_for('dashboard.dashboard_page'))


@dashboard.route('/dashboard/user/category/<int:category_id>/activity/<int:activity_id>/delete')
@token_check
...
@dashboard.route('/dashboard/user/category/<int:category_id>/delete')
@token_check
def delete_category(user_id, category_id):
    ...
    for category_check in categories:
        if category_check['user_id'] == user_id and category_check['category_id'] == int(category_id):
            ...
            with open('app/data/users_categories.json', 'w+') as categories_file:
                json.dump(categories, categories_file, indent=2)
            return redirect(url_for('dashboard.dashboard_page'))


@dashboard.route('/dashboard/user/category/<int:category_id>/activity/create', methods=['POST', 'GET'])
@token_check
def create_activity(user_id, category_id):
    user_name = get_username(user_id)
    form = CreateActivityForm(request.form)
    if request.method == 'POST' and form.validate():
        with open('app/data/users_activities.json', 'r+') as activity_files:
        ...
        for activity_check in activities:
            if request.form['activity_name'].strip() == activity_check['activity_name'] and \
                ...
                return redirect(url_for('dashboard.create_activity', category_id=category_id))
        ...
        return redirect(url_for('dashboard.dashboard_page'))
    return render_template('dashboard/create_activity_page.html', user_name=user_name, form=form)


@dashboard.route('/dashboard/user/category/create', methods=['POST', 'GET'])
@token_check
def create_category(user_id):
    ...
    if request.method == 'POST' and form.validate():
        ...
        with open('app/data/users_categories.json', 'w+') as categories_file:
            json.dump(categories, categories_file, indent=2)
        return redirect(url_for('dashboard.dashboard_page'))
    return render_template('dashboard/create_category_page.html', user_name=user_name, form=form)
